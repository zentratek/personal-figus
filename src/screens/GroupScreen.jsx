  ...
  return (
    <AppLayout title="GRUPO">
      <div className="p-4 pb-24 space-y-6">
        {/* Group Header */}
        <div className="p-6 rounded-2xl bg-[var(--surface-2)] border-2 border-[var(--border)]">
          ...
          {/* Group Code */}
          <div className="p-4 bg-[var(--surface)] rounded-xl border-2 border-[var(--border)] mb-4">
            <p className="text-xs text-[var(--muted)] mb-2 text-center">Código del Grupo</p>
            <div className="text-2xl font-bold text-[var(--lime)] text-center font-mono tracking-wider">
              {group.code}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={copyCode}
              className="px-4 py-3 bg-[var(--surface-3)] border-2 border-[var(--border)] rounded-lg font-bold hover:bg-[var(--border)] transition-colors flex items-center justify-center gap-2"
            >
              <span>📋</span> Copiar Código
            </button>
            <button
              onClick={shareCode}
              className="px-4 py-3 bg-[var(--cyan)] text-black border-2 border-black rounded-lg font-bold hover:translate-y-[-2px] transition-transform flex items-center justify-center gap-2"
            >
              <span>📤</span> Invitar
            </button>
          </div>
        ...
        {/* Group Stats */}
        <div className="p-6 rounded-2xl bg-[var(--surface-2)] border-2 border-[var(--border)]">
          <h3 className="text-sm font-bold mb-4 text-[var(--muted)]">ESTADÍSTICAS DEL GRUPO</h3>
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              <div className="text-3xl font-bold text-[var(--lime)] mb-1">
                {group.stats.averageCompletion}%
              </div>
              <div className="text-xs text-[var(--muted)]">Promedio</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-[var(--cyan)] mb-1">
                {group.stats.totalOwned}
              </div>
              <div className="text-xs text-[var(--muted)]">Tenemos</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-[var(--muted)] mb-1">
                {group.stats.totalNeeded}
              </div>
              <div className="text-xs text-[var(--muted)]">Faltan</div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {group.members
              ...
              .map((member) => (
                <div
                  key={member.userId}
                  className="p-4 rounded-xl bg-[var(--surface-2)] border-2 border-[var(--border)] flex items-center gap-4"
                >
                  {/* Avatar */}
                  <div className="w-12 h-12 rounded-full bg-[var(--surface-3)] overflow-hidden flex-shrink-0 border-2 border-[var(--border)]">
                    {member.photoURL ? (
                      <img
                        src={member.photoURL}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {member.userId === user.uid && (
                        <span className="text-[var(--muted)] text-sm">(Vos)</span>
                      )}
                    </div>
        ...
        <div className="pt-4">
          <button
            ...
            className="w-full px-4 py-3 bg-[var(--surface-2)] border-2 border-[var(--red)] text-[var(--red)] rounded-lg font-bold hover:bg-[var(--red)]/20 transition-colors"
          >
            Salir del Grupo
          </button>
        </div>
